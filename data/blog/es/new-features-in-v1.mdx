...
Migrating from v1? You can revert to the previous theme by setting `primary` to `colors.sky` (Tailwind 2.2.2 and above, otherwise `colors.lightBlue`) and changing neutral to `colors.neutral`.
...
.highlight-line {
  @apply -mx-4 border-l-4 border-primary-500 bg-neutral-700 bg-opacity-50;
}

.line-number::before {
  @apply -ml-2 mr-4 inline-block w-4 text-right text-neutral-400;
  content: attr(line);
}
```
